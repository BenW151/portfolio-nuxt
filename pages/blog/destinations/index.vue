<template>
  <div class="destination-pages">
    ...
    <p v-else>No destinations found or failed to load destinations.</p> <!-- Fallback if no destinations -->
  </div>
</template>
...
const { data: destinations } = await useAsyncData("destinations", async () => {
  try {
    // Fetch all destination pages from the 'blog/destinations' folder
    const content = await queryContent("blog/destinations").find(); // Update path to 'blog/destinations'
